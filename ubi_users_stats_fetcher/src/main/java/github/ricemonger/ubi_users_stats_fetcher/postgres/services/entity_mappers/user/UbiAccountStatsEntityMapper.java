...
public class UbiAccountStatsEntityMapper {
    ...
    public List<UbiAccountStatsEntity> createEntities(List<UbiAccountStats> ubiAccounts) {
        List<String> existingItems = customItemPostgresRepository.findAllItemIds();
        ...
        for (UbiAccountStats ubiAccount : ubiAccounts) {
            ...
            List<ItemEntity> ownedItemsIds = existingItems.stream().filter(ex -> ubiAccount.getOwnedItemsIds().contains(ex)).map(customItemPostgresRepository::getReferenceById).toList();
            ...
            for (ItemResaleLock resaleLock : ubiAccount.getResaleLocks()) {
                ItemEntity item = existingItems.stream().filter(ex -> ex.equals(resaleLock.getItemId())).map(customItemPostgresRepository::getReferenceById).findFirst().orElse(null);
                if (item != null) {
