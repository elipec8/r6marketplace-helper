...
import github.ricemonger.trades_manager.services.TelegramBotService;
import github.ricemonger.trades_manager.services.UbiAccountService;
...
public class ScheduledAllUbiUsersManager {
    ...
    public void fetchAllUbiUsersStatsAndManageTrades() {
        List<UserUbiAccountEntry> userUbiAccountEntries = ubiAccountService.findAllUsersUbiAccountEntries();

        List<UbiAccountStats> updatedUbiAccountsStats = new ArrayList<>();

        for (UserUbiAccountEntry userUbiAccountEntry : userUbiAccountEntries) {
            updatedUbiAccountsStats.add(fetchAndGetUserPersonalStats(userUbiAccountEntry));
        }

        commonValuesService.setLastUbiUsersStatsFetchTime(LocalDateTime.now().withNano(0));

        ubiAccountService.saveAllUbiAccountStats(updatedUbiAccountsStats.stream().map(UbiAccountStatsEntityDTO::new).toList());

        centralTradeManager.manageAllUsersTrades(updatedUbiAccountsStats);
    }

    private UbiAccountStats fetchAndGetUserPersonalStats(UserUbiAccountEntry userUbiAccountEntry) {
        AuthorizationDTO authorizationDTO = userUbiAccountEntry.getAuthorizationDTO();
        ...
        List<UbiTrade> currentBuyTrades = currentOrders.stream().filter(order -> order.getCategory().equals(TradeCategory.Buy)).toList();
            ...
            notifyUser(userUbiAccountEntry, creditAmount, soldIn24h, boughtIn24h);

        return new UbiAccountStats(
                userUbiAccountEntry.getProfileId(),
    ...
    private void notifyUser(UserUbiAccountEntry userUbiAccountEntry, int creditAmount, List<UbiTrade> soldIn24h, List<UbiTrade> boughtIn24h) {

        boolean creditsChanged = userUbiAccountEntry.getCreditAmount() != null && userUbiAccountEntry.getCreditAmount() != creditAmount;
        ...
        if (creditsChanged || soldIn24hChanged || boughtIn24hChanged) {
            ...
            if (creditsChanged) {
                message.append("Your credit amount has changed. Old:").append(userUbiAccountEntry.getCreditAmount()).append(" New : ").append(creditAmount).append("\n");
            }
            if (soldIn24hChanged) {
            ...
            telegramBotService.sendPrivateNotification(userUbiAccountEntry.getUserId(), message.toString());
        }
    }
