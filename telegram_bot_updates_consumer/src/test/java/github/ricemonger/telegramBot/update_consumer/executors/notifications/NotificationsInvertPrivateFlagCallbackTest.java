...
@SpringBootTest
class NotificationsInvertPrivateFlagCallbackTest {
    @MockBean
    private BotInnerService botInnerService;

    @Test
    public void initAndExecute_should_invert_flag_and_send_text() {
        NotificationsInvertPrivateFlagCallback command = new NotificationsInvertPrivateFlagCallback();
        command.initAndExecute(MockUpdateInfos.UPDATE_INFO, botInnerService);

        verify(botInnerService).invertUserPrivateNotificationsFlag(MockUpdateInfos.UPDATE_INFO.getChatId());
