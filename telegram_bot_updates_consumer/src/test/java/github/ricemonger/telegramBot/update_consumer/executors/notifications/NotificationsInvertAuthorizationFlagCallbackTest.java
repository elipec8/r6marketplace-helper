...
@SpringBootTest
class NotificationsInvertAuthorizationFlagCallbackTest {
    @MockBean
    private BotInnerService botInnerService;

    @Test
    public void initAndExecute_should_invert_flag_and_send_text() {
        NotificationsInvertAuthorizationFlagCallback command = new NotificationsInvertAuthorizationFlagCallback();
        command.initAndExecute(MockUpdateInfos.UPDATE_INFO, botInnerService);

        verify(botInnerService).invertUserAuthorizationNotificationsFlag(MockUpdateInfos.UPDATE_INFO.getChatId());
