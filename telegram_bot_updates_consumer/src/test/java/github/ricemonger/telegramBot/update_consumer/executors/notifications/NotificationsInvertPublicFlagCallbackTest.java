...
@SpringBootTest
class NotificationsInvertPublicFlagCallbackTest {
    @MockBean
    private BotInnerService botInnerService;

    @Test
    public void initAndExecute_should_invert_flag_and_send_text() {
        NotificationsInvertPublicFlagCallback command = new NotificationsInvertPublicFlagCallback();
        command.initAndExecute(MockUpdateInfos.UPDATE_INFO, botInnerService);

        verify(botInnerService).invertUserPublicNotificationsFlag(MockUpdateInfos.UPDATE_INFO.getChatId());
